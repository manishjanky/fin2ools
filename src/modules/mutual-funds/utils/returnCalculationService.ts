...
export class ReturnCalculationService {
  ...
  static async calculateAndStoreSchemeReturns(
    ...
    if (!investments || investments.length === 0 || !navHistory || navHistory.length === 0) {
      return;
    }
    ...
    const oneDayChange = calculateOneDayChange(navHistory, { schemeCode, investments });
  ...
  static async calculateAndStorePortfolioReturns(
    ...
    const portfolioCagr = calculateCAGRForInvestments(allInvArray, mergedNavHistory);

    // Calculate portfolio-level 1-day change
    const portfolioOneDayChange = calculatePortfolioOneDayChange(schemeNavHistories, allInvestments);
